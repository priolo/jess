...
test("synchronize two CLIENTS with the SERVER", async () => {
	...
	expect(server.objects["my-object"].value).toHaveLength(3)
	expect(client1.getObject("my-object").value).toHaveLength(3)
	expect(client2.getObject("my-object").value).toHaveLength(3)
})
...
test("[II] cannot handle try in a JEST test! the CLIENT starts offline", async () => {
	...
	// Expect init to fail because onSend is not defined/connected
	try {
		await client.init("my-object", true)
	...
	// Expect update to fail because onSend is not defined
	try {
		await client.update()
		throw new Error("Should have thrown")
	} catch (e) {
